...
const secondTab =({navigation}) => {
  ...
  const [latestGrade, setLatestGrade]=useState([0,0,0,0,0,0,0,0]);
  const {emailID} = useContext(AuthContext);

  useEffect(() => {
    async function firePromise() {
      try {
        const snap = await firestore().collection(emailID).orderBy('Count','desc').limit(1).get();
        snap.forEach(doc=> {
          if(doc.id!==emailID){
          tempDoc[0]=doc.get('GradeCard');
      ...
      howmany>1 ?
          <ScrollView style={styles.homeElementView}>
              ...
              <View style={{borderWidth:0.5, marginBottom:15, borderColor : '#b2b2b2', backgroundColor : '#fbfbfb'}}>
                {InhalerType>0 && latestGrade.map((mem, key)=>{
                  if(mem===2){
                    return <Text key={key} style={{fontSize:15, padding:3}}>{checklistContents[InhalerType-1][key]}</Text>
                  }
                })}
              </View>
              <Text style={{fontSize:20, marginBottom : 10, color : '#235EC3', fontWeight : "600"}}>{emailID}님이 잘 사용하신 단계는</Text>
